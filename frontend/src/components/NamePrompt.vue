...
<template>
	<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="handleCancel">
		<div class="bg-white rounded-lg shadow-lg p-6 min-w-80 max-w-md w-full mx-4">
			<h3 class="text-lg font-semibold mb-4 text-neutral-800">{{ title }}</h3>
			<input
				ref="inputRef"
				v-model="name"
				type="text"
			...
			<div class="flex justify-end gap-2 mt-4">
				...
				<button
					@click="handleSubmit"
					class="px-4 py-2 text-sm bg-blue-600 text-white hover:bg-blue-700 rounded-md transition-colors cursor-pointer"
				>
					{{ buttonText || 'Create' }}
				</button>
			</div>
